  ...
  mount(rootEl, ticker) {
    ...
    genBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        // 자동 커밋: GitHub PAT 있으면 adhoc 모드로 repo에 저장
        // (새로고침해도 사라지지 않도록)
        if (localStorage.getItem('sma_github_pat')) {
          autoCommit().catch(e => {
            commitSt.textContent = '자동 저장 실패 (수동 저장 버튼 사용): ' + e.message;
          });
        } else {
          commitSt.textContent = '※ GitHub PAT 저장하면 자동 아카이브됨 (대시보드에서 설정)';
        }
      } catch (e) {
    ...
    async function autoCommit() {
      ...
      if (!ghPat || !lastMarkdown) return;
      commitSt.textContent = '📝 repo에 자동 저장 중...';
      const today = new Date().toISOString().slice(0,10);
      // 같은 날 여러 번 생성 시 덮어쓰기 방지를 위해 adhoc 파일은 시각 suffix
      const now = new Date();
      const hh = String(now.getUTCHours()).padStart(2,'0');
      const mm = String(now.getUTCMinutes()).padStart(2,'0');
      const filename = `${today}-adhoc-${hh}${mm}.html`;
      const path = `reports/tickers/${ticker}/narratives/${filename}`;
      const page = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${ticker} AI Narrative · ${today} (adhoc)</title></head><body style="background:#FFFFFF;color:#1F2328;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:24px;"><div style="max-width:1000px;margin:0 auto;"><div style="font-size:11px;color:#656D76;margin-bottom:12px;"><a href="../../../tickers/${ticker}/" style="color:#0969DA;text-decoration:none;">← ${ticker} page</a> · Generated by Claude Sonnet 4.5 (adhoc) · ${today} ${hh}:${mm} UTC</div>${sanitizeHTML(lastMarkdown)}</div></body></html>`;
      const content = btoa(unescape(encodeURIComponent(page)));
      const res = await fetch(`https://api.github.com/repos/z66g/sma/contents/${path}`, {
        method: 'PUT',
        headers: { 'Authorization': 'Bearer ' + ghPat, 'Accept': 'application/vnd.github+json' },
        body: JSON.stringify({ message: `narrative: ${ticker} ${today} adhoc`, content })
      });
      if (!res.ok) {
        const err = await res.json().catch(()=>({}));
        throw new Error(err.message || res.statusText);
      }
      commitSt.innerHTML = `✅ 저장됨 → <a href="narratives/${filename}">${filename}</a> · 1-2분 후 위 아카이브 목록에 자동 반영`;
    }

    // 수동 저장 버튼 (autoCommit 재호출)
