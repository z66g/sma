    ...
    options:{{ responsive:true, maintainAspectRatio:false,
      plugins:{{
        legend:{{
          ...
          labels:{{
            usePointStyle:true, pointStyle:'line',  // 범례를 선 모양 + dash 반영
            boxWidth:30, boxHeight:0, padding:12
          }}
        }},
        tooltip:{{ usePointStyle:true }}
      }},
      scales:{{y:{{grid:{{color:'{COLOR['panel']}'}}}},x:{{grid:{{display:false}}}}}},
      elements:{{point:{{radius:2, pointStyle:'circle'}},line:{{tension:0.3,borderWidth:2}}}},
    }}
  }});
}}
